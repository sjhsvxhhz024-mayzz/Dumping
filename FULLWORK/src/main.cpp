...
// FIX bug #3: базис камеры строим ИСКЛЮЧИТЕЛЬНО из МИРОВОГО кватерниона
// m_LookRoot. Offline-анализ (Unity 6000.3.18f1) показал:
//   * m_LookRoot.rotation(world) = yaw(родитель) * pitch(локально) = ПОЛНАЯ
//     ориентация камеры. Это единственный корректный, полный источник.
//   * Поля 0x60(kqP)/0x90(_kqm) — это ДВА разных аккумулированных Vector2
//     input-угла (getter'ы uaA/uaR), которые движок сам применяет к трансформам
//     через WPr.GTv(). Складывать их с уже-полным кватернионом lookRoot =
//     ДВОЙНОЙ pitch -> боксы «уезжают вверх при взгляде вверх». Поэтому
//     отдельный pitch-угол БОЛЬШЕ НЕ используется.
// eyePos = мировая позиция m_LookRoot (истинный глаз камеры).
static bool ox_readLookBasis(uint64_t player, Vector3 &right, Vector3 &up,
                             Vector3 &fwd, Vector3 *eyePos, bool full) {
    Quat q; bool have = false;
    ...
    if (ox_isPtr(mouseLook)) {
        ...
        Vector3 lrPos;
        if (ox_transformWorld(lookRoot, &lrPos, &q, false, nullptr)) {
            float n2 = q.x*q.x + q.y*q.y + q.z*q.z + q.w*q.w;
            if (n2 > 0.8f && n2 < 1.2f) { have = true; if (eyePos) *eyePos = lrPos; }
        }
    }
    // Fallback: worldCameraRoot (0x68) — тоже несёт полную ориентацию камеры
    // (в этом риге держит yaw+pitch). Позицию из него НЕ берём (rig-высота).
    if (!have) {
        uint64_t camRoot = rpm<uint64_t>(player + ox::PM_TRANSFORM);
        if (!ox_transformWorld(camRoot, nullptr, &q, false, nullptr)) return false;
        float n2 = q.x*q.x + q.y*q.y + q.z*q.z + q.w*q.w;
        if (n2 < 0.8f || n2 > 1.2f) return false;
    }

    // Нормализуем кватернион (mem-снимок мог быть чуть денормализован).
    {
        float n = sqrtf(q.x*q.x + q.y*q.y + q.z*q.z + q.w*q.w);
        if (n > 1e-6f) { q.x/=n; q.y/=n; q.z/=n; q.w/=n; }
    }

    right = quatRotate(q, Vector3(1, 0, 0));
    up    = quatRotate(q, Vector3(0, 1, 0));
    fwd   = quatRotate(q, Vector3(0, 0, 1));
    ox_normalize3(right); ox_normalize3(up); ox_normalize3(fwd);
    if (full) OXLOGT("  lookBasis have=%d fwd=(%.3f,%.3f,%.3f) up=(%.3f,%.3f,%.3f)",
                     (int)have, fwd.x, fwd.y, fwd.z, up.x, up.y, up.z);
    return true;
}
...
static CameraView ox_getCamera(uint64_t elems, int count, bool full) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        // FIX bug #2: стабильный team-key (raw teamName UTF-16, иначе pf.teamId).
        g_localTeamName = ox_teamKey(player);
        if (full) OXLOGI("  local teamKey='%s' (empty => not in a team)", g_localTeamName.c_str());
...
static void ox_updateBuildingCache(bool full) {
    ...
    if (full) OXLOGI("[LOS] кэш построек: %d валидны из %d обойдённых", valid, total);
    {
...
static void ox_runAimbot(const CameraView& cam, const std::vector<PlayerCacheData>& players) {
    ...
    for (const PlayerCacheData& player : players) {
        ...
        // Никогда не наводимся на сокомандника (team-key совпал). player.teamName
        // хранит стабильный team-key (см. UpdatePlayerCache / ox_teamKey).
        if (!g_localTeamName.empty() && !player.teamName.empty() &&
...
void UpdatePlayerCache() {
    ...
    for (int i = 0; i < size; i++) {
        ...
        // FIX bug #2: стабильный team-key (raw UTF-16 teamName / pf.teamId).
...
static void oxTabAim() {
    ...
    ImGui::Checkbox(oxorany("Crouch-safe aim (enemies crouching)"), &aimCrouchSafe);
    if (aimCrouchSafe) {
        ImGui::SliderFloat(oxorany("Crouch drop (m)"), &aimCrouchDrop, 0.20f, 0.70f, "%.2f");
        ImGui::TextColored(TEXT_DIM, oxorany("Lowers aim point. Crouch state is NOT"));
        ImGui::TextColored(TEXT_DIM, oxorany("readable externally - toggle manually."));
    }
...
static void oxTabCamera() {
    ...
    ImGui::Spacing();
    // camFov = ГОРИЗОНТАЛЬНЫЙ FOV. Unity Camera.fieldOfView вертикальный (~55-60)
    // -> на ландшафтном телефоне горизонтальный ~95-105. tanV=tanH*(H/W) в W2S.
    ImGui::SliderFloat(oxorany("Horizontal FOV"), &camFov, 60.0f, 120.0f, "%.2f");
    ImGui::TextColored(TEXT_DIM, oxorany("Horizontal FOV. Tune until boxes lock at screen edges."));
    ImGui::TextColored(TEXT_DIM, oxorany("Calibrated default: 98.61 (vFOV~60 @ 20:9)."));
    ImGui::Spacing();
    ImGui::Checkbox(oxorany("Invert pitch"), &camInvertPitch);
    ImGui::Checkbox(oxorany("Invert yaw"),   &camInvertYaw);
    ImGui::Spacing();
    ImGui::TextColored(TEXT_DIM, oxorany("Camera basis now from m_LookRoot world quaternion"));
    ImGui::TextColored(TEXT_DIM, oxorany("(full yaw+pitch). Legacy pitch/swap toggles removed."));
}
