...
// По просьбе erafox — держим ВСЁ, что пишет чит, внутри отдельной папки
// /sdcard/Download/EclipsOxide/, чтобы Download не засорялся.
static const char* CANDIDATE_DIRS[] = {
    "/sdcard/Download/EclipsOxide",
    "/storage/emulated/0/Download/EclipsOxide",
    "/sdcard/EclipsOxide",
    "/data/local/tmp/EclipsOxide",
    // Fallbacks (root path без подпапки) — если поддиректория не создаётся из-за прав.
    "/sdcard/Download",
...
// Публичный корень для всего, что чит пишет наружу (dump, config, screenshots).
// Заполняется в init() тем каталогом, где удалось открыть лог.
static char g_activeDir[192] = {0};

static void ensureDir(const char* path) {
    // Пробуем pre-create по цепочке: /sdcard, /sdcard/Download, /sdcard/Download/EclipsOxide.
    // Игнорируем ошибки — mkdir прекрасно возвращает EEXIST.
    char tmp[192];
...
const char* init() {
    ...
    for (const char* dir : CANDIDATE_DIRS) {
        // рекурсивно создаём каталог со всеми промежуточными уровнями
        // (mkdir(2) не создаёт цепочку сам, а /sdcard/Download/EclipsOxide/ обычно ещё не существует).
        ensureDir(dir);
        char full[256];
        ...
        if (f) {
            ...
            snprintf(g_activeDir, sizeof(g_activeDir), "%s", dir);
            break;
        }
    }
