...
bool Touch_Init(int w, int h, uint32_t orientation_, bool readOnly) {
    ...
    if (!dir) {
        fprintf(stderr, "Touch: cannot open /dev/input: %s\n", strerror(errno));
        return false;
    }
    dirent *ptr = NULL;
    int eventCount = 0;
    ...
    for (i = 0; i <= eventCount; i++) {
        ...
        fd = open(temp, readOnly ? O_RDONLY : O_RDWR);
        if (fd < 0) {
            continue;
        }
    ...
    if (minCnt > eventCount) {
        fprintf(stderr, "Touch: no readable touchscreen in /dev/input (Android 12/cloud fallback unavailable)\n");
        return false;
    }
...
static bool checkDeviceIsTouch(int fd) {
    ...
    free(bits);
    // Some Android 12 and virtual/cloud touch devices expose tracking IDs and
    // coordinates but no slot axis. The reader already supports slot 0.
    return itmp2 && itmp3;
}
