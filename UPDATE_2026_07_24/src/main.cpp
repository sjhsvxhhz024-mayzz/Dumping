...
static constexpr uint64_t OX_META_NAME_OFF_PLAYERMANAGER = 0x171ee6;  // "PlayerManager" (was 0x1704f0)
static constexpr uint64_t OX_META_NAME_OFF_BUILDINGPIECE = 0x10c379;  // "BuildingPiece" (was 0x10c22b)
static constexpr uint64_t OX_META_NAME_OFF_PLAYERVITALS  = 0x16aee6;  // "PlayerVitals"  (was 0x169546)
...
static constexpr uint64_t OX_META_MC_RVA         = 0xbf80ff8;
static constexpr uint64_t OX_META_BASE_PTR_RVA   = 0xbf81008;
static constexpr uint64_t OX_META_HEADER_PTR_RVA = 0xbf81010;
static constexpr uint64_t OX_META_STRIDE_RVA     = 0xbf8101c;
...
static constexpr uint32_t OX_TYPEDEF_COUNT        = 29486; // was 29366

// Fast-seed через s_TypeInfoTable[TDI]. Детерминированный, 3 vm_readv на класс.
static bool ox_fastSeedTypeInfoFromHeader() {
    if (!il2cpp_base) return false;

    // Читаем s_TypeInfoTable pointer из libil2cpp writable BSS slot @ 0xBF80FF0
    // (UPDATE 2026/07/24). NULL если MetadataCache::Initialize ещё не отработал
    // (крайне маловероятно — Initialize вызывается при загрузке libil2cpp.so,
    // ещё до появления главного меню игры).
...
static void oxDrawCounter(const ImVec2& ds) {
    ...
    // Иконка-прицел слева: встроенный PNG "crosshair" (тинт = accent, пульс —
    // как у прежнего вектора). Fallback на вектор, если текстура не загружена.
    ImVec2 ic(a.x + 30.f * s, a.y + H * 0.5f);
    float ir = 11.f * s * (1.0f + pulse * 0.15f);
    if (g_iconTexCrosshair) {
        // слот совпадает по габариту с прежним кольцом (радиус ir + вылет штрихов ~4*s)
        float half = ir + 4.f * s;
        dl->AddImage(g_iconTexCrosshair, ImVec2(ic.x - half, ic.y - half),
                     ImVec2(ic.x + half, ic.y + half), ImVec2(0, 0), ImVec2(1, 1), accent);
    } else {
...
void Layout_tick_UI() {
    ...
    if (g_menuOpen) {
        ...
        // ── шапка: логотип-значок + название + FPS-чип ──────────────────────
        {
            ...
            // Бренд-значок: встроенный PNG badge, тинт accent, в том же круглом
            // слоте (радиус lr). Мягкая аура сохранена. Fallback — прежний вектор.
            dl->AddCircleFilled(lc, lr, Fade(g_pal.accent, 0.20f));
            if (g_iconTexBrandBadge) {
                dl->AddImage(g_iconTexBrandBadge, ImVec2(lc.x - lr, lc.y - lr),
                             ImVec2(lc.x + lr, lc.y + lr), ImVec2(0, 0), ImVec2(1, 1),
                             g_pal.accent);
            } else {
            ...
            char t1[16]; snprintf(t1, sizeof(t1), "%s", oxorany("Eclips Oxide"));
            char t2[8];  snprintf(t2, sizeof(t2),  "%s", oxorany(" NEW"));
            ...
            dl->AddText(hf, ImGui::GetFontSize() * 0.5f, ImVec2(tp.x, tp.y + titleSz + 2 * s * 0.34f),
                        g_pal.textFaint, oxorany("survival island"));
        ...
        if (portrait) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                TabIcon(dl, ImVec2(cxp, y0 + navH * 0.40f), i, col, 0.85f * s * 0.34f + 0.6f);
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                TabIcon(dl, ImVec2(x0 + 34 * s * 0.34f + 12, iy + itemH * 0.5f), i,
        ...
        {
            char wm[40]; snprintf(wm, sizeof(wm), "%s  %.0f", oxorany("Eclips Oxide NEW"), io.Framerate);
