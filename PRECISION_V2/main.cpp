...
static void oxTabAim() {
    ...
    ImGui::Checkbox(oxorany("Only when aiming (ADS/scope)"), &aimOnlyADS);
    ImGui::TextColored(TEXT_DIM, oxorany("Aims only while scoped. Detected from live FOV zoom."));
...
static void oxTabCamera() {
    using namespace oxui;
    ImGui::TextColored(ACCENT, oxorany("Camera / W2S"));
    ImGui::Spacing();
    ImGui::TextColored(TEXT_DIM, oxorany("Basis from look angles (kqP pitch/yaw)."));
    ImGui::TextColored(TEXT_DIM, oxorany("FOV read live from game (vertical, ADS-aware)."));
    ImGui::Spacing();
    ...
    ImGui::TextColored(TEXT_DIM, oxorany("Fallback FOV (used only if live FOV unavailable):"));
    ImGui::SliderFloat(oxorany("Fallback H-FOV"), &camFov, 60.0f, 120.0f, "%.1f");
}
