...
// Fast-seed через ЦЕЛЕВОЙ ПОИСК ссылки на имя класса. Использует чистую
// offline-цепочку: metadata_base из runtime + hardcoded offset имени из
// decrypted global-metadata → runtime name VA → поиск слота в libil2cpp writable.
// НИКАКИХ сканов анон-памяти игры. Bulk read libRw, локальный поиск.
static bool ox_fastSeedTypeInfoFromHeader() {
    if (!il2cpp_base) return false;
    // Нужен metadata_base — читаем через chain (использует OX_META_BASE_PTR_RVA)
    uint64_t meta = rpm<uint64_t>(il2cpp_base + OX_META_BASE_PTR_RVA);
    if (!ox_isPtr(meta)) {
        OXLOGD("[fast-seed] metadata_base ещё не установлен");
        return false;
    }

    // Собираем libil2cpp writable regions (маленькие ~5-15 МБ)
    std::vector<OxMapRange> libRwMaps;
    ox_collectMaps(libRwMaps, ox::MODULE, true, 32ULL * 1024 * 1024);
    if (libRwMaps.empty()) {
        OXLOGD("[fast-seed] libil2cpp writable maps not found");
        return false;
    }
    ...
    Seed seeds[] = {
        { OX_META_NAME_OFF_PLAYERMANAGER, "PlayerManager", &g_playerManagerKlass },
        { OX_META_NAME_OFF_BUILDINGPIECE, "BuildingPiece", &g_buildingPieceKlass },
    };
    int seeded = 0;
    for (const Seed &s : seeds) {
        uint64_t nameVa = meta + s.nameOff;
        uint64_t klass = ox_findKlassByNameVa(nameVa, libRwMaps);
        if (!klass) {
            OXLOGD("[fast-seed] %s: слот с nameVa=0x%llx не найден в libil2cpp writable",
                   s.expected, (unsigned long long)nameVa);
            continue;
        }
        // Финальная проверка: читаем имя из klass+CLASS_NAME и сверяем.
        char cn[96] = {0};
        ox_className(klass, cn, sizeof(cn));
        if (cn[0] == 0 || strcmp(cn, s.expected) != 0) {
            OXLOGD("[fast-seed] %s: klass=0x%llx имя='%s' — не подтвердился",
                   s.expected, (unsigned long long)klass, cn);
            continue;
        }
        *s.klass_out = klass;
        ++seeded;
        OXLOGI("[fast-seed] %s: klass=0x%llx (via name-VA 0x%llx=metadata+0x%llx) — OK",
               s.expected, (unsigned long long)klass,
               (unsigned long long)nameVa, (unsigned long long)s.nameOff);
    }
